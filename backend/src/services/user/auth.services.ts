...
export const signup = async (req:Request,res:Response): Promise<any> => {
  ...
  let userFound = await user.findOne({where:{$or:[{email:payload.email},{wallet_address:payload.address}]}});
...
export const login = async (req:Request,res:Response): Promise<any> => {
  ...
  let userFound = await user.findOne({where:{$and:[{email:payload.email},{wallet_address:payload.address}]},
    attributes: {
