...
getFollowers();
  ...
  const Follow=async(id:any)=>{
    await contract.methods.Follow(id).send({from:accounts})
  }
  return (
        ...
        {follow.map((item:any, index) => (
          <div key={index} className={style.item}>
            ...
            <div className={style.profileDetails}>
              <div className={style.name}>{item?.id?.slice(0,4)}</div>
              {/* <div className={style.handle}>{item.handle}</div> */}
            </div>
            <div className={style.followButton} onClick={()=>Follow(item.id)}>Follow</div>
          </div>
        ))}
        <div className={style.showMore}>Show more</div>
      </div>
